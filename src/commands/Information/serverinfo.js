...
exports.run = async (bot, message, args) => {
  ...
  .addField("**Statuses:**", `Online: ${message.guild.members.cache.filter(s => s.user.presence.status === "online").size}\nIdle: ${message.guild.members.cache.filter(s => s.user.presence.status === "idle").size}\nDo Not Disturb: ${message.guild.members.cache.filter(s => s.user.presence.status === "dnd").size}\nOffline: ${message.guild.members.cache.filter(s => s.user.presence.status === "offline").size}`)
  .addField(`**Channels [${message.guild.channels.cache.filter(c => c.type === "text" || c.type === "voice").size}]:**`, `Text: ${message.guild.channels.cache.filter(c => c.type === "text").size}\nVoice: ${message.guild.channels.cache.filter(c => c.type === "voice").size}\nCategory: ${message.guild.channels.cache.filter(c => c.type === "category").size}`, true)
